...
inline void argMaxWarpCheck(int N, int K, const double *aHost, const Pair *bHost) {
    for (int k = 0; k < K; ++k) {
...
// Do not edit.
inline void argMaxBlockCheck(int N, int K, const double *aHost, const Pair *bHost) {
    for (int k = 0; k < K; ++k) {
...
inline void testSmallArgMax(Kernel kernel, CheckFunc checkFunc, int div, int N) {
    int K = (N + div - 1) / div;
    double *aHost;
    Pair *bHost;
    double *aDev;
    ...
    CUDA_LAUNCH(kernel, blocks, threads, aDev, bDev, N);
...
inline void testLargeArgMax(const char *name, Func func, int N) {
    double *aHost;
    double *aDev;
    Pair *bDev;
